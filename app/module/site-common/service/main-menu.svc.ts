...
import { ConfigUpdate_Mdl, GlobalConfig_Mdl } from 'lm/site-common';
...
export class MainMenu_Svc {
  ...
  constructor(private configSvc: Config_Svc) {
    ...
    this.configSvcSub = this.configSvc.globalConfigUpdate
      .subscribe((data: ConfigUpdate_Mdl) => {
        this.onConfigUpdate(<GlobalConfig_Mdl>data.config);
      });
  }
  ...
  private onConfigUpdate(data: GlobalConfig_Mdl) {
    this.mainMenuCfg = data.component.mainMenu || {};
