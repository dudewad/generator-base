...
export class Config_Svc {
  ...
  loadAppCfg(cfgUrl: string) {
    ...
    this.http.get(cfgUrl)
      .subscribe(result => {
        ...
        error => {
          //TODO: Don't throw an error, catch it and redirect to a 404 page
          throw new Error(`Critical error - could not load site route map. The `
                          + `resource '${cfgUrl}' could not be found.`);
        });
  }


  private loadGlobalConfig() {
    const url = this.constants.url.dataRoot + this.configs.app['globalConfig'];
  ...
  private loadPageCfg() {
    ...
    if (routes.hasOwnProperty(path)) {
      ...
      //TODO: No default and no recognized path. Bail/404 instead of error
    ...
    this.http.get(url)
      .subscribe(result => {
        ...
        error => {
          ...
          console.warn(`Error retrieving page JSON file for ${url}. `
